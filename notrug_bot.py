...
{check(twitter_follow)} Follow @NOTRUGfun → *+20 points*
→ /submit\\_follow Twitter\\_kullanici\\_adin

{check(pinned_retweet)} Retweet pinned tweet → *+25 points*
→ /submit\\_retweet tweet\\_linki

{check(pinned_comment)} Comment on pinned tweet → *+15 points*
...
{check(telegram_join)} Join the group → *+10 points*
→ /submit\\_join
...
🪙 Tokens to earn: *{tokens} NOTRUG*
🏆 Rank: *{rank}*
💼 Wallet: `{wallet}`

━━━━━━━━━━━━━━━━━━━━
{'✅ Max points reached!' if points >= MAX_POINTS else f'📈 {MAX_POINTS - points} more points to go!'}
{'🎯 You will earn 2,000 NOTRUG!' if points >= MAX_POINTS else ''}
...
async def wallet_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(wallet) < 32 or len(wallet) > 44:
        await update.message.reply_text("❌ Invalid Solana address! Please check.")
        return
...
async def submit_retweet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not verify_tweet(tweet_url):
        await update.message.reply_text("❌ Invalid tweet link!")
        return
...
async def submit_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ok:
        await update.message.reply_text("❌ This tweet was already submitted!")
        return
    ...
    if current_points >= MAX_POINTS:
        await update.message.reply_text(
            "🏆 Max points reached! But keep earning for the leaderboard!"
        )
...
def main():
    ...
    print("🛡️ NOTRUG Airdrop Bot started!")
    app.run_polling()
